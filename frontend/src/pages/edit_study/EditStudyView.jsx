...
function EditStudyView() {
  ...
  const fetchPDFPath = async (id) => {
    ...
    if (data.pathto_pdf) {
      const filePath = data.pathto_pdf.startsWith('/') ? data.pathto_pdf : `/${data.pathto_pdf}`
      ...
      try {
        ...
        if (headResponse.ok && contentType === 'application/pdf' && contentLength !== '0') {
          setFileUrl(filePath)
        } else {
          console.warn('PDF file does not exist or invalid file type:', filePath)
          setFileUrl("") 
        }
      } catch (error) {
  ...
  return (
    <div className="flex flex-row h-screen bg-violet-50">
        ...
        <StudyForm studyid={params.studyid} refreshPdf={() => fetchPDFPath(params.studyid)} />    
        </div>
        {fileUrl &&  <div className=" w-2/4 h-full">
